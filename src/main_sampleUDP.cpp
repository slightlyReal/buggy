...
ESP_LOGE(TAG, "Error: %s",  "This is an error message");   // Error level (Red)
ESP_LOGW(TAG, "Warning: %s", "This is a warning message");  // Warning level (Yellow)
ESP_LOGD(TAG, "Debug: %s", This is a debug message");   // Debug level (Blue)
ESP_LOGV(TAG, "Verbose: %s", "This is a verbose message"); // Verbose level (White)

*/

const char *ssid = WIFI_SSID;
const char *password = WIFI_PASSWORD;
const char *hostname = "ESP32-1"; // Only letters, numbers, and dashes allowed
...
void udpSenderTask(void *pvParameters)
{
  ...
  while (1)
  {
    ...
    packet_length = snprintf(payload, sizeof(payload), "Packet length %d : #%u from ESP32, elapsed_us : %lld", packet_length, counter++, elapsed_us);
...
void setup()
{
  ...
  WiFi.setHostname(hostname); // 1. Set custom hostname
  WiFi.mode(WIFI_STA);        // 2. Set station mode : ESP32 will be connected to Hotspot as Wi-Fi client (station)
